...
enum class MethodTextGenerator : Function<Iterable<PsiField>, String> {
  ...
  Read {
    override fun apply(fields: Iterable<PsiField>): String {
      ...
      fields.forEach { builder.append(generate(it, Function<FieldGenerator, BiFunction<Optional<String>, PsiType, String>> { FieldGenerator.read(it) })) }
      builder.append('}')
  ...
  Write {
    override fun apply(fields: Iterable<PsiField>): String {
      ...
      fields.forEach { builder.append(generate(it, Function<FieldGenerator, BiFunction<Optional<String>, PsiType, String>> { FieldGenerator.write(it) })) }
      builder.append('}')
      return builder.toString()
    }
  };

  protected final fun generate(field: PsiField, operation: Function<FieldGenerator, BiFunction<Optional<String>, PsiType, String>>): String {
    val type = field.type
