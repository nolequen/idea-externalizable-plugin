...
class CleanupMethods : PsiClassModification {
  ...
  override fun accept(psiClass: PsiClass) {
    ...
    Stream.of(*psiClass.methods)
        .filter(
            MethodNameFilter("readExternal").and(SingleParameterTypeFilter(project, "java.io.ObjectInput"))
                .or(MethodNameFilter("writeExternal").and(SingleParameterTypeFilter(project, "java.io.ObjectOutput"))))
        .forEach { it.delete() }
  }
  ...
  private class SingleParameterTypeFilter(project: Project, type: String) : Predicate<PsiMethod> {
    ...
    init {
      this.type = TypeFinder(project, type).get()
    }
